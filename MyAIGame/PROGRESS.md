...
## 다음에 진행할 작업
- [기반] Display 해상도 640x360 확인, F5 실행 확인 (사용자 PC 필요)

## 버그 및 주의사항
- **에디터 검증 1차 통과 (Godot 4.7.2, 윈도우).** 확인한 것만 TASKS 에 체크했다.
  - 인풋 맵 6개 정상. 손으로 쓴 형식이 그대로 먹었다
  - 출력 패널 에러 0개. `.tres` 20개가 전부 읽혔다
  - Main 아래 Camera2D 정상, Zoom 1.0
  - 폴더 구조와 .gitignore 정상
- **아직 확인 못 한 것**
  1. Display 해상도가 640x360 인가
  2. 스트레치 모드 viewport / 종횡비 keep 이 적용됐는가
  3. 렌더러가 Compatibility 인가
  4. Nearest 텍스처 필터가 적용됐는가
  5. F5 실행이 되는가 (시작 씬 지정 확인 겸)
- 2D 뷰에서 루트 노드 `Main` 을 드래그하면 위치가 바뀌어 경고가 뜬다.
  씬 > 저장된 씬 되돌리기 로 복구한다. 루트 노드는 옮기지 않는다.
