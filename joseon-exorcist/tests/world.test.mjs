...
test('현재 임무: 흑석촌 → 여우골 → 서낭고개 → 폐사찰 → 큰고을 조사 준비 중', () => {
  ...
  assert.deepEqual([Core.currentMission(after3).region, Core.currentMission(after3).ready], ['pyesachal', true]);
  const after4 = { v: 1, cleared: ['ch1', 'ch2', 'ch3', 'ch4'], merit: 19 };
  assert.deepEqual([Core.currentMission(after4).region, Core.currentMission(after4).ready], ['keungoeul', false]);
});

test('장 무대 지역: 흑석촌 1장, 여우골 2장, 서낭고개 3장, 폐사찰 4장, 나머지는 전투 없음', () => {
  assert.deepEqual(Core.REGIONS.filter((r) => r.chapter).map((r) => [r.id, r.chapter]), [['heukseok', 'ch1'], ['yeougol', 'ch2'], ['seonang', 'ch3'], ['pyesachal', 'ch4']]);
