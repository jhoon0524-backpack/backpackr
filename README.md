...
- **긴 회의도 OK**: 업로드 한도(약 25분)를 넘는 녹음은 10분 단위로 자동 분할해 전사하고 결과를 이어붙입니다.
- **실패해도 재녹음 불필요**: 네트워크 오류 등으로 중간에 실패하면 기록 목록의 **재시도** 버튼으로 남은 단계부터 이어서 처리합니다. 완료된 기록도 전사문이 있으면 **재생성**으로 회의록만 다시 만들 수 있습니다.
...
> ⚠️ 다른 앱을 보는 중이거나 화면이 꺼진 상태에서 볼륨 키로 시작한 녹음은 Android 11+의 마이크 보호 정책 때문에 기기에 따라 **무음으로 저장될 수 있습니다**. 확실한 녹음은 사이드 키(빠른 녹음) 연동을 사용하세요.
