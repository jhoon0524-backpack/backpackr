...
"작업실" 방향 — 잡지의 작품 목록처럼. 종이색 바탕, 먹색 글자, 명조 제목, 가는 괘선, 기울여 찍은 "N자리 남음" 도장.
텀블벅의 피치색은 하나뿐인 강조색으로만 남겼다. 토큰은 `app/globals.css`, 버튼·입력칸·칩 모양은 `app/ui.ts` 한 곳에 있다.
다른 두 방향(게시판·스케치북)은 디자인 캔버스에 나란히 있다 (`PROGRESS.md` 4차).
