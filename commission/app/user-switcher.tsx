...
export async function UserSwitcher() {
  ...
  return (
    ...
    */
    <form action={switchUser} className="flex h-9 items-center gap-1 border-[3px] border-white/40 bg-ink px-2">
      <select
        name="userId"
        ...
        aria-label="사용자 고르기"
        className="h-full appearance-none bg-ink px-1 text-[13px] font-bold text-white focus:outline-none"
      >
      ...
      <button type="submit" className="h-full px-1 text-[13px] font-bold text-white hover:text-yellow">
        바꾸기
      </button>
    </form>
  )
