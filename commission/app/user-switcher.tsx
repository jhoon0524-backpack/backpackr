...
export async function UserSwitcher() {
  ...
  return (
    ...
    */
    <form action={switchUser} className="flex h-9 items-center border-[3px] border-white">
      <select
        name="userId"
        ...
        aria-label="사용자 고르기"
        className="h-full appearance-none bg-ink px-2.5 text-[13px] font-bold text-white focus:outline-none"
      >
        <option value="">보는 사람 고르기</option>
        {users.map((u) => (
          <option key={u.id} value={u.id}>{u.nickname}</option>
        ))}
      </select>
      <button type="submit" className="h-full border-l-[3px] border-white bg-white px-2.5 text-[13px] font-bold text-ink">
        전환
      </button>
    </form>
  )
