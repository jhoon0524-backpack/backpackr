...
export async function UserSwitcher() {
  ...
  return (
    ...
    */
    <form action={switchUser} className="flex h-9 items-center gap-3 divide-x-[3px] divide-white/40">
      <select
        name="userId"
        ...
        aria-label="사용자 고르기"
        className="h-full appearance-none bg-ink pr-3 text-[15px] font-bold text-white/60 hover:text-white focus:outline-none"
      >
      ...
      <button type="submit" className="h-full pl-3 text-[15px] font-bold text-white/60 hover:text-white">
        바꾸기
      </button>
    </form>
  )
