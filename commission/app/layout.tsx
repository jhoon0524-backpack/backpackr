        ...
        <header className="sticky top-0 z-10 border-b border-line bg-white">
          <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between gap-4 px-5">
            <div className="flex items-center gap-8">
              <Link href="/" className="flex items-center gap-1.5 text-[22px] font-extrabold tracking-tight text-ink">
              ...
              <nav className="hidden gap-6 text-[15px] font-medium text-ink sm:flex">
                {NAV.map((n) => (
                  <Link key={n.href} href={n.href} className="hover:text-accent-deep">{n.label}</Link>
                ))}
              </nav>
            </div>
            <div className="flex items-center gap-3">
              <Link href="/open" className="hidden text-[15px] font-medium text-ink hover:text-accent-deep sm:inline">
                커미션 열기
              </Link>
              <UserSwitcher />
            </div>
