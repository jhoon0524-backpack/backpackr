...
export function HeaderNav({ items }: { items: readonly { href: string; label: string }[] }) {
  ...
  return (
    <nav className="disp hidden gap-8 text-[18px] sm:flex">
      {items.map((n) => {
        ...
        return (
          ...
          >
            <span className={`flex items-center ${here ? 'border-b-2 border-yellow pb-1.5 text-white' : 'border-b-2 border-transparent pb-1.5 text-white/45 hover:text-white'}`}>
              {n.label}
            </span>
          </Link>
        )
