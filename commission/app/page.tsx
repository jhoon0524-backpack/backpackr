  ...
  return (
    <div>
      ...
      <section className="relative left-1/2 -mx-[50vw] w-screen border-b-[3px] border-ink bg-ink">
        <div className="mx-auto max-w-[1200px] px-8 pb-20 pt-5">
          ...
          <h1 className="poster -ml-[0.05em] mt-5 whitespace-nowrap text-[min(14.1vw,202px)] tracking-[-0.035em] text-white">
            {openSlots}자리 남았어요
          </h1>
          {/*
      ...
      {commissions.length === 0 ? (
        <ul className="relative z-10 -mt-9 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <li><EmptySlot /></li>
        </ul>
      ) : (
  ...
  return (
    <section className="mt-14">
      ...
      */}
      <ol className="mt-6">
        {STEPS.map((s) => (
          <li key={s.n} className="flex items-baseline gap-6 py-5 not-last:border-b-[3px] not-last:border-ink">
            <span className="poster num w-12 shrink-0 text-[46px] leading-none text-ink">{s.n}</span>
            <span className="w-32 shrink-0 text-[17px] font-extrabold text-ink">{s.t}</span>
...
function EmptySlot() {
  return (
    ...
    >
      ...
      <span className="disp mt-auto flex items-center justify-between border-t-[3px] border-dashed border-ink pt-3 text-[17px] text-ink">
        메뉴 붙이기
