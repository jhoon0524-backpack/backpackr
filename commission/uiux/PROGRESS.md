**판정: 통과** (2026-09-04, 표 v1, 화면 6개, 실패 0건)

6회차. 실패 수 **15 → 7 → 5 → 2 → 2 → 0.**
검사표가 "실패 0건이면 통과, 그때 이 화면은 끝이다" 라고 정해 두었으므로 **여섯 화면은 여기서 끝이다.**

## 통과에 딸린 단서 — 확인 불가 3자리

통과는 "실패 0건" 이지 "다 봤다" 가 아니다. 마지막 채점자가 캡처로 판단할 수 없다고 적은 것이 셋이다.
**다음 회차 표에 그대로 넣는다.**

1. **D2 / 의뢰 상세** — 결과물 주소 칸(`type="url"`)에 공백만 넣으면 앱 문구 대신
   **브라우저 기본 말풍선**이 뜨는 것으로 보인다. 말풍선은 캡처에 안 찍히고, 언어와 모양이 앱과 다르며 곧 사라진다.
   빈 값으로 보낸 쪽(14번)은 앱 문구가 떠서 D2 는 그쪽으로 통과했다
2. **E2 / 단추의 기다리는 표시** — "보내는 중…" 을 확인한 단추는 의뢰 보내기 하나뿐이다.
   메뉴 붙이기·고친 내용 저장·수락 확정·거절 확정·전달 확정·의뢰 취소는 누른 뒤 기다리는 순간의 캡처가 없다
3. **C4 / 초점이 눈에 보이는가** — Tab 으로 닿는 것과 초점 외곽선의 굵기는 코드로 쟀지만,
   **검은 머리띠 위에서 1px 외곽선이 실제로 보이는지**는 캡처가 없다

## 다음 회차 표에 넣을 것 (채점자들이 회차마다 남긴 것)

새 항목의 조건은 "이걸 어긴 걸 어떻게 알지?" 에 한 문장으로 답할 수 있는 것이다. 아래는 그 조건을 만족한다.

| 항목 후보 | 어긴 걸 어떻게 알지 |
|---|---|
| 오류 문구가 전부 앱이 그린 것인가 (브라우저 기본 말풍선이 없는가) | 각 칸에 공백만 넣고 보내 본다 |
| 기다리는 표시가 곧 뜰 화면의 뼈대와 같은 모양인가 | 지금은 여섯 화면 모두 "카드 3장" 뼈대다. 폼 화면에도 카드가 뜬다 |
| 기다리는 동안 단추를 다시 누를 수 없게 **보이는가** | 누른 뒤 단추의 색·모양이 평소와 다른가 |
| 빈 화면의 다음 할 일이 **그 화면을 보는 사람**에게 맞는가 | 메뉴 0건 화면의 길은 "메뉴 붙이기"(창작자용)뿐이다. 처음 온 의뢰인에게는 길이 없다 |
| 필수 입력에 최소 길이가 적혀 있고 지켜지는가 | 메뉴 제목 2자를 보내 본다 (지금 통과한다) |
| 되돌릴 수 있어도 **남에게 즉시 보이는** 행동은 확인을 받는가 | 내리기를 누른다. 지금은 바로 내려간다 |
| 켜진 길이 **들어온 길**과 같은가 | 내 의뢰 → 메뉴 고치기로 들어오면 머리에서 "메뉴판" 이 켜진다 |
| 확인 상자의 확정 단추가 한 가지 말투인가 | 수락·거절·전달은 "…확정" 인데 취소만 "의뢰 취소" 다 |
| 확인 단계 **전에** 입력을 검사하는가 | 빈 전달 폼은 "전달 확정" 까지 누른 뒤에야 오류가 난다 |
...
| 회차 | 실패 | 무엇이 걸렸나 |
|---|---|---|
| 1 | **15** | 대비 3건, 크기 2건, 간격 2건, 현재 위치 2건, 이름 2건, 나머지 4건 |
| 2 | **7** | 좁은 화면 넘침, 손해 보는 숫자, 고정 막대, 오류 위치 2건, 이름 2건 |
| 3 | **5** | 오류 위치 1건, 다음 할 일 1건, 이름 1건, 단위 2건 |
| 4 | **2** | 처리 중 단추 대비, 취소에만 없던 확인 단계 |
| 5 | **2** | 같은 자리 수를 두 방향으로, 같은 행동을 두 동사로 |

**재서 확인되는 것은 3회차에서 끝났다.** 대비·크기·간격·가로 스크롤·44px 는 4회차부터 한 건도 안 나온다
(4회차의 대비 1건은 정적 화면이 아니라 **처리 중** 단추라 그전까지 아무도 재지 않던 자리였다).
남는 것은 전부 **말**과 **빠뜨린 한 자리**다 — 원칙은 세워 두고 한 군데만 안 지킨 곳.

## 회차마다 배운 것

**1회차 — 점수를 버리니 셀 수 있는 답이 나왔다.** 40바퀴를 5~6점 사이에서 돌던 자리에서 15건이 특정됐다.

**2회차 — 캡처가 모자라면 통과가 아니라 "확인 불가"다.** 1회차에 15자리가 확인 불가였고,
그중 하나는 내가 "오류 화면" 이라고 넘긴 것이 실은 **성공 화면**이었다. 채점자가 그걸 잡았다.
그 뒤로는 오류를 만들 때 `required` 를 떼는 대신 **서버가 실제로 거부하는 값**을 쓴다.

**3회차 — 같은 실수를 반대 방향으로 했다.** 이번엔 "성공" 이라고 넘긴 것이 오류 화면이었다.
그래서 캡처를 찍은 뒤 **오류 상자가 있는지 코드로 확인**하고 그 값을 채점자에게 함께 준다.

**4·5회차 — 남는 실패는 "다른 데는 다 있는데 여기만 없다" 꼴이다.**
확인 단계는 수락·거절·전달·물리기에 다 붙였는데 **취소에만** 없었다.
값 표기는 한 가지로 맞췄는데 **처리 중 단추**만 흐려져 대비가 무너졌다.
자리 수는 "남은 자리" 로 통일했는데 **상세 화면의 표 한 칸**만 반대 방향으로 세고 있었다.
사람이 화면을 훑어서는 잘 안 보이고, 항목별로 세니까 나온다.

## 채점자에게 주는 것 (5회차 기준)

캡처 54장 — 여섯 화면 × 폭 1440·390, 어두운 테마 6장, 마감된 메뉴 4장, 폭 390 뷰포트 맨 아래 2장,
빈 화면 4장, 오류 6장, 확인 단계 4장, 누른 뒤 6장, 느린 연결 7장, 의뢰인 시점 2장.

그리고 **브라우저에서 직접 잰 값**을 사실로 함께 준다 — 가로 스크롤, 44px 미만인 누르는 것,
8px 미만으로 붙은 쌍, 상자 밖으로 넘친 글자, 화면마다의 Tab 경로와 초점 표시.
캡처만으로는 이것들을 눈대중할 수밖에 없고, 눈대중은 회차마다 답이 달라진다.
