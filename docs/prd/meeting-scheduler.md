...
| RabbitMQ 알림 이벤트 + DLQ 재처리 | 트랜잭션 커밋 후 동기 메일 발송, 실패 시 `sync_error` 기록 + 담당자 알림 메일 | 메일 하루 몇 통. 큐를 태우면 발송 실패 관측·재처리 화면이 딸려온다 |
...
**규모**: 테이블 3개(v1) / 5개(v2), 엔드포인트 13개, 화면 7개.
...
- 캘린더 미연동 또는 토큰 철회 상태면 **예약 페이지를 열지 않고** "예약이 일시 중단되었습니다"를 노출하고 담당자에게 알림 메일을 보낸다. busy를 모르는 채로 예약을 받으면 이중 부킹이 나므로 열지 않는 쪽이 안전하다.
...
- **시간 단위로 미팅을 막는 수단은 담당자의 구글 캘린더다.** 이동 시간·점심·개인 일정 무엇이든 캘린더에 일정으로 넣으면 freebusy가 그 구간을 busy로 돌려주고 슬롯에서 빠진다. 담당자가 원하는 때에, 원하는 길이로, 반복 일정까지 그대로 쓴다.
- 앞뒤 여유 시간을 자동으로 비우는 설정은 **두지 않는다.** 모든 미팅에 일괄로 걸리는 규칙이라 담당자가 "언제"를 지정할 수 없고, 여유가 필요 없는 날까지 슬롯을 깎는다. 필요한 날 캘린더에 블록을 넣는 쪽이 더 정확하고 코드가 0줄이다.
...
- 메일 발송이 실패하면 예약은 유효한 상태로 두고 실패를 예약에 기록한 뒤 담당자에게 알림 메일을 보낸다. 자동 재시도는 하지 않는다. 메일 서비스 장애면 이 알림도 함께 실패하므로, 어드민 예약 목록의 실패 뱃지가 최종 안전망이다.

*맞춤 예약 링크 (v1)*
- 미팅 상대가 이미 정해진 경우가 대부분이다. 담당자가 화면 ①에서 상대의 이름·회사명·이메일·연락처를 넣어 **맞춤 링크**를 만들면, 상대는 시간만 고르고 확정한다.
- 값은 **링크의 쿼리 파라미터로 싣는다.** `/booking/{slug}?name=&company=&email=&phone=` — 서버에 미리 저장하지 않는다.
- **개인정보를 미리 저장하지 않는 것이 이 설계의 핵심이다.** 담당자가 입력한 값을 서버에 먼저 만들어두면 본인 동의 없이 개인정보를 보관하게 된다. 링크에만 실으면 저장 시점은 여전히 상대가 확정 버튼을 누르는 순간이고, 동의 주체도 본인 그대로다. 테이블·컬럼·엔드포인트가 하나도 늘지 않는다.
- 필수 4개가 모두 채워져 오면 2단계를 **요약 카드**로 보여준다. 상대가 할 일은 내용 확인 · 동의 · 확정 세 번이다. 하나라도 비어 오면 평소 폼을 띄우되 채워진 값은 채워둔 채로 시작한다.
- 요약 카드에 **"담당자가 미리 입력한 정보입니다"**를 밝히고 수정 경로를 함께 둔다. 자기 정보가 이미 적혀 있는 화면은 밝히지 않으면 불쾌감을 준다.
- 링크를 잘못 보내면 그 안의 정보가 다른 사람에게 보인다. 담긴 값은 담당자가 이미 갖고 있던 리드 정보이고 위험 수준은 **메일 오발송과 같다.** 별도 방어를 두지 않는다.
- 기존 공용 링크는 그대로 둔다. 파라미터가 없으면 지금 동작 그대로다.

*예약 페이지 UI (v1)*
- 날짜 선택은 **월 달력**을 쓴다. 예약 창 14일이 두 달에 걸치면 나머지 날짜가 다음 달에 있으므로, 창 안에 남은 날이 있을 때만 `›`를 활성화한다. 14일 전용 날짜 스트립은 신규 컴포넌트가 되어 FE 제약(기존 컴포넌트만 사용)에 걸린다.
- 페이지 진입 시 **예약 가능한 가장 빠른 날짜를 자동 선택**한다. 빈 슬롯 패널로 시작하지 않는다. 예약 창에 열린 날이 하나도 없을 때만 빈 상태를 노출한다.
- 확정 화면의 1차 액션은 **캘린더에 추가**다. 구글 초대는 예약자가 메일에서 수락해야 캘린더에 들어가므로, 확정 직후가 가장 확실한 시점이다. 취소는 텍스트 링크로 내린다 — 방금 확정한 사람에게 가장 큰 버튼이 취소일 이유가 없다.
- 캘린더 추가는 **프런트에서 `.ics` 문자열을 만들어 내려받는다.** 서버 엔드포인트를 만들지 않는다. 확정 화면이 이미 필요한 값(시각·제목·`meeting_url`·담당자)을 들고 있고, `.ics`는 구글·아웃룩·애플에 모두 붙는다.
- 예약 중단·취소 불가 화면의 담당자 연락처는 `mailto:`·`tel:` 링크로 건다. **이 화면에 도달한 사람에게 남은 행동은 그 하나뿐이라** 주소를 받아적게 두면 이탈한다.
- KST 표기는 화면당 1회, 달력 옆에만 둔다.
- 날짜·시간 버튼에는 스크린리더용 레이블을 단다. 숫자만 읽히면 날짜를 분간할 수 없다 — `7월 28일 화요일, 예약 가능`.
...
- Salesmap API 호출이 실패해도 예약은 유효하게 두고 담당자에게 알림 메일을 보낸다. 캘린더와 달리 CRM 기록은 사후 보완이 가능하므로 예약을 막지 않는다.
...
flowchart TD
    ...
    B --> C{캘린더 연동 정상?}
    C -->|아니오| C1[예약 일시 중단 안내<br/>담당자 알림 메일]
    ...
    K --> N[양측 확정 메일]
    N -.발송 실패.-> N1[예약은 유효<br/>실패 기록 + 담당자 알림]
```
...
| 캘린더 토큰 만료·권한 철회 | 예약 페이지 중단 + 담당자 알림 메일. busy를 모르는 채로 예약받지 않는다 |
...
| 메일 발송 실패 | 예약과 캘린더 이벤트는 유효. 예약에 실패 기록 + 담당자 알림 메일 후 수동 재발송 |
...
- [ ] 담당자가 캘린더에 임의의 시간 블록을 넣으면 그 구간이 슬롯에서 빠지고, 블록을 지우면 다시 열린다.
- [ ] 맞춤 링크로 들어오면 2단계가 요약 카드로 뜨고, 동의 후 확정까지 추가 입력이 없다. 수정을 누르면 폼으로 바뀌고 값이 유지된다.
- [ ] 파라미터 없이 들어온 공용 링크는 기존과 동일하게 빈 폼으로 동작한다.
...
- [ ] 확정 화면에서 받은 `.ics`가 구글·아웃룩·애플 캘린더에 등록되고 시각·제목·회의실 링크가 일치한다.
- [ ] 예약 중단·취소 불가 화면의 담당자 연락처가 링크로 동작한다.
