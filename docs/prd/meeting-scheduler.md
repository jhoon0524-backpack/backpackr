...
| 라운드로빈 팀 배정 | 담당자 개인 링크 | 배정 규칙·부하 분산·가용성 합산이 전부 딸려온다 |
...
- `guest_company`·`guest_phone`은 영업 리드 정보라 수집한다. 개인정보 수집 항목이 늘어난 만큼 동의 문구와 파기 정책이 v1 필수다.
...
- [ ] 동의 없이 예약 확정이 불가하고, 수집 항목이 동의 문구에 그대로 열거되어 있다.
...
**논의사항**
- 외부 고객 개인정보(이름·회사·이메일·연락처) 보관 기간 — 미팅 종료 후 1년 자동 삭제로 제안. 사내 상담용이 아니라 **대외 수집**이므로 법무 확인이 v1 릴리즈 조건이다.
- 예약 데이터가 CRM(Salesmap)에 들어가지 않는다. v1은 담당자가 수동 입력하고, 실사용 2주 후 자동 생성 필요 여부를 판단한다.
- 해외 리드 대상 미팅이 있으면 KST 고정이 문제가 된다. 국내 리드만이면 그대로 간다 — **영업팀 확인 필요.**
- 담당자별 개인 링크로 시작한다. "아무 담당자나 가능한 시간"이 필요해지면 그때 라운드로빈을 검토한다.
