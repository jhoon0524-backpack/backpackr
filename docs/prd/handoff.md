...
**맞춤 링크에도 엔드포인트가 없다.** 프리필 값은 쿼리 파라미터로만 오간다. 서버는 11번에서 넘어온 값을 평소처럼 받을 뿐이고, 담당자가 링크를 만드는 화면도 문자열 조립이라 프런트에서 끝난다.

**확정 화면의 "캘린더에 추가"용 엔드포인트는 없다.** `.ics`는 프런트에서 문자열로 만들어 내려받는다. 11번 응답이 이미 시각·제목·`meeting_url`·담당자를 들고 있어서 서버를 한 번 더 부를 이유가 없다.
...
- 1번의 정렬 기준은 **구간 시작점**이다. 자정이나 정시가 아니다. 10:00–12:00 / 30분이면 10:00·10:30·11:00·11:30, 10:15–11:45 / 30분이면 10:15·10:45·11:15다.
- 3번의 freebusy 호출은 요청당 1회다. 날짜별로 나눠 부르지 않는다.
- **활성 예약과 캘린더 busy를 같은 구간 목록으로 합쳐 4번 한 곳에서 판정한다.** 예약을 시각 일치로 따로 걸러도 지금은 결과가 같지만, 겹침 판정이 두 벌이면 한쪽만 고치는 실수가 난다. 판정 코드는 하나만 둔다.
