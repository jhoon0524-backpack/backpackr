...
- [x] pg_cron 잡 정의 작성 — 1분 주기 마감 처리, 실행·처리 건수 기록 (등록은 사람이)
...
- [~] 운영자 검수 — DB 함수(`approve_product`/`reject_product`)와 테스트는 완료. HTTP 라우트는 남음
...
- [~] 알림 트리거 연결 — 낙찰·유찰·미결제는 상태 변경 트랜잭션에서 적재하도록 완료.
      상위 입찰·결제 기한 3시간 전·반려는 남음

## [화면]
- [~] 드롭 목록 — 마감 임박순·빈 화면 동작. 스켈레톤·실패(재시도) 상태는 남음
