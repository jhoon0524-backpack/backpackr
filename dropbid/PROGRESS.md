...
## 다음에 진행할 작업
- [데이터] DB 테스트 하네스 셋업 — 지금까지의 확인이 전부 수동이라 자동 테스트로 고정해야 한다
...
- **Auth·Storage·Realtime 은 이 환경에서 한 번도 안 돌려봤다.** Docker 가 되는 환경에서 확인해야 한다.
  ...
  여기서 완료로 표시하면 안 된다
- **스키마 제약과 `place_bid` 동작 전부 수동으로만 확인했다.** 인수 조건(동시 입찰 100건)도 손으로 돌렸다.
  자동 테스트로 고정되기 전에는 누가 고쳐도 깨진 걸 알 수 없다. 다음 작업이 이것인 이유다
