...
## 사람에게 물어봐야 하는 것
- **연장 상한 20회가 경매당인가 회차당인가.** PRD 기능 명세는 "회차당 최대 20회" 라고 쓰여 있는데,
  엣지케이스 표는 "연장이 20회에 도달한 뒤 추가 입찰" 이라 경매당으로 읽힌다.
  일단 **경매당**으로 구현했다 (`auctions.extension_count`). 회차당이 맞다면 스키마를 바꿔야 한다.
  결정 전까지 `place_bid` 작업을 이 가정 위에 쌓는 것에 주의할 것
