...
- [데이터] pg_cron 잡 정의 + 스케줄러 기록 테스트 6개 — 전체 64개 통과. **[데이터] 12개 항목 전부 끝**
  ...
  - 로컬에 pg_cron 확장이 없어서 **cron 등록 자체는 검증하지 못했다.** 함수는 직접 호출해 검증했다
...
## 다음에 진행할 작업
- [기능] 카카오 로그인 연동 — **다만 이 환경에서는 Supabase Auth 를 띄울 수 없다.**
  아래 "확인하지 못한 것" 참고. Docker 가 되는 환경이 필요하다
...
- PRD 인수 조건 9개 중 7개가 테스트로 고정됐다 (동시 입찰, 30초 연장, 후원 인증 누락 거부,
  ...
  남은 2개는 유찰·낙찰 알림 발송과 결제 기한 만료 — 둘 다 [기능] 단계 작업이다
