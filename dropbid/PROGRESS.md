...
## 다음에 진행할 작업
- [데이터] `place_bid` 동시성 테스트 — 동일 금액 동시 입찰 100건 중 1건만 성공 (PRD 인수 조건)
...
- **Auth·Storage·Realtime 은 이 환경에서 한 번도 안 돌려봤다.** Docker 가 되는 환경에서 확인해야 한다.
  ...
  여기서 완료로 표시하면 안 된다
- **동시 입찰 인수 조건(100건 중 1건만 성공)은 아직 수동 확인뿐이다.** 다음 작업이 이것을 고정한다.
  릴리즈 1단계의 롤백 조건이 "동시 입찰 인수 조건 미통과" 라서 자동화가 특히 중요하다
